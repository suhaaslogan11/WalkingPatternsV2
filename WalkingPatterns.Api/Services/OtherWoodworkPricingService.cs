...
public class OtherWoodworkPricingService : IOtherWoodworkPricingService
{
    ...
    public async Task<OtherWoodworkItemResponse?> UpdateOrderAsync(int projectId, int orderId, OtherWoodworkItemRequest request)
    {
        ...
        if (project == null || order == null || !string.Equals(order.UtilityNameOld, "Other Woodwork", StringComparison.OrdinalIgnoreCase)) return null;
        await using var tx = await _context.Database.BeginTransactionAsync();
        var created = await CalculateAndSaveAsync(projectId, request); if (created == null) return null;
        var temp = await _context.OtherWoodworkPriceDetails.FindAsync(created.Id); if (temp == null) return null;
        ...
        _context.OtherWoodworkPriceDetails.Remove(temp);
