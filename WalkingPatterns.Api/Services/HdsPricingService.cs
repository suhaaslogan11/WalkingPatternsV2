...
public class HdsPricingService : IHdsPricingService
{
    ...
    public HdsPricingResponse GetPricing()
    {
        ...
        var file = JsonSerializer.Deserialize<HdsPricingFile>(
            stream,
        ...
        return file?.PricingData ?? new HdsPricingResponse();
    }

    public async Task<HdsItemResponse?> CalculateAndSaveAsync(
        int projectId,
        HdsItemRequest request)
    {
        ...
        var parent = request.Parent?.Trim() ?? "HDS";
        ...
        if (string.IsNullOrWhiteSpace(shutter))
        {
        ...
        else if (!pricing.Items.TryGetValue(shutter, out materialTotal))
        {
            errors["shutterMaterial"] = new[]
            {
                "Selected HDS material/service is not available."
            };
        ...
        request.AdditionalItems ??=
            new List<HdsAdditionalItemRequest>();
        ...
        {
            throw new HdsValidationException(errors);
        }
        ...
        var entity = new HDSPriceDetails
        {
            Parent = parent,
        ...
        _context.HDSPriceDetails.Add(entity);

        await _context.SaveChangesAsync();

        return new HdsItemResponse
        {
            Id = entity.Id,
...
public sealed class HdsValidationException : Exception
{
    public Dictionary<string, string[]> Errors { get; }

    public HdsValidationException(
        Dictionary<string, string[]> errors)
        : base("HDS input validation failed.")
    {
        Errors = errors;
    }
}
